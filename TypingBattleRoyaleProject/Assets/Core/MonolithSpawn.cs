...
public class MonolithSpawn : NetworkBehaviour
{
    ...
    {
        Debug.Log($"[MonolithSpawn] OnNetworkSpawn — IsServer:{IsServer} IsHost:{IsHost}");
        if (!IsServer) return;
        SpawnMonolith();
    }
    public void SpawnMonolith()
    {
        ...
        for (int i = 0; i < amountToSpawn; i++)
        {
            ...
            {
                Debug.LogError("[MonolithSpawn] El prefab NO tiene NetworkObject");
                return;
            }

            networkObject.Spawn(true);

            Debug.Log($"[MonolithSpawn] Spawneado: {monolith.name} en {selectedPoint.position}");
        }
    }
}
